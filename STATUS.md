...
- **App:** LIAM Control Centre **v1.69.0** — local-first, truth-stated
...
- **AI brain:** six LLM providers — groq · gemini · openrouter · deepseek · mistral (free-tier keys via `connect <id> with token <key>`) and ollama (local, no key); `ask <anything>` answers labelled `provider · model`; `ask all` ensembles every connected provider at once; unmatched chat falls back to the brain; missing keys are truth-stated with the free-key path; Ollama is loopback-11434-only; uninstalled local models fall back to an installed one, named
...
- **Tests:** spec 93 · adversarial 78 · platform 144 · arena 28 · engagement 117 · smoke 57 — **517 checks, 0 failures**
